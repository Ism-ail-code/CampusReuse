...
import { ChevronLeft, ChevronRight, ImagePlus, Loader2, X, Heart, ShoppingCart, Repeat2 } from "lucide-react"
...
import { CATEGORIES, CONDITIONS, EDUCATION_LEVELS, SUBJECTS } from "@/lib/constants"
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <button
            type="button"
            onClick={() => set("transactionType", "sell")}
            className={cn(
          ...
          >
            <span className={cn("flex items-center gap-2 text-sm font-semibold", values.transactionType === "sell" ? "text-primary" : "text-foreground")}>
              <ShoppingCart className="h-4 w-4" aria-hidden />
              Sell
            </span>
            <span className="mt-1 block text-xs text-muted-foreground">Set a price and sell your item.</span>
          </button>
          <button
            type="button"
            onClick={() => set("transactionType", "exchange")}
            className={cn(
              "rounded-xl border p-4 text-left transition-colors",
          ...
          >
            <span className={cn("flex items-center gap-2 text-sm font-semibold", values.transactionType === "exchange" ? "text-primary" : "text-foreground")}>
              <Repeat2 className="h-4 w-4" aria-hidden />
              Exchange
            </span>
            <span className="mt-1 block text-xs text-muted-foreground">Swap it for something you need.</span>
          </button>
          <button
            type="button"
            onClick={() => set("transactionType", "donate")}
            className={cn(
              "rounded-xl border p-4 text-left transition-colors",
              values.transactionType === "donate"
                ? "border-primary bg-primary/5 ring-1 ring-primary"
                : "hover:border-ring",
            )}
          >
            <span className={cn("flex items-center gap-2 text-sm font-semibold", values.transactionType === "donate" ? "text-primary" : "text-foreground")}>
      ...
      {values.transactionType === "donate" && (
        <div className="rounded-lg border bg-emerald-50 p-4 text-sm text-emerald-700">
          ...
          <p className="mt-1">This item will appear in the Get Support section. Students who need it can contact you directly.</p>
        </div>
      )}
